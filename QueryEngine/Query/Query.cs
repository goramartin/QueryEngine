...
namespace QueryEngine
{
    ...
    sealed class Query
    {
        ...
        private Query(List<Token> tokens, Graph graph, int threadCount, string printer, string formater, int verticesPerThread, string fileName, bool isStreamed)
        {
            this.graph = graph;
            this.variableMap = new VariableMap();
            this.qEhelper = new QueryExecutionHelper(threadCount, printer, formater, verticesPerThread, fileName, "DFSParallelStreamed", "DFSSingleThreadStreamed", "SIMPLE", "twowayHSL");
